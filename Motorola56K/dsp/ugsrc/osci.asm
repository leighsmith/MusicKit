...
;;
;;  SOURCE
;;      /usr/local/lib/dsp/ugsrc/osci.asm
;;
;;  SEE ALSO
;;      /usr/local/lib/dsp/ugsrc/oscg.asm - osc with general power-of-two table
;;      /usr/local/lib/dsp/ugsrc/oscs.asm = oscg with no mask in inner loop (fastest)
;;      /usr/local/lib/dsp/ugsrc/oscgf.asm = oscgaf without an amplitude envelope.
;;
;;  ALU REGISTER USE
...
; Set up data alu regs from state held in memory arguments
	    ...
	    fail 'First rewrite to remove usage of obsolete R_L register'
