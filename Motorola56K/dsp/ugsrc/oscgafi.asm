...
;;                 12 instructions.
;;  30/01/93/daj - Changed to use new L arg pointer -- David Jaffe
;;
;;------------------------------ DOCUMENTATION ---------------------------
;;  NAME
...
;;      phs  = l:(LARGS)P_L;         /* current table read pointer (phs1,phs0) */
;;
;;      for (n=0;n<I_NTICK;n++) {
;;           phs1 &= mtab;
;;           phs2 = (phs1+1) & mtab;
...
;;
;;  SOURCE
;;      /usr/local/lib/dsp/ugsrc/oscgafi.asm
;;
;;  TEST PROGRAM
;;
;;
;;  SEE ALSO
;;      /usr/local/lib/dsp/ugsrc/oscgaf.asm - same as this one but non-interpolating
;;
;;  ALU REGISTER USE
...
; Set up data alu regs from state held in memory arguments
          ...
          move y:(R_Y)+,N_O        ; Input increment signal pointer. Put it here for now
  	  ...
  	  move x:LARGS,R_I1	   ; Get L arg pointer
          ...
          move l:(R_I1),B10        ; Instantaneous phase

          move R_X,x:(R_I1)        ; Save R_X for later restoral
          move R_Y,y:(R_I1)        ; Save R_Y for later restoral
	  move N_O,R_I1	           ; Now put signal pointer here.
...
pf\_oscgafi_\ic\_wave1 
  	  move x:LARGS,R_I1	   ; Get L arg pointer
	  nop
          move x:(R_I1),R_X      ; Restore R_X
          move y:(R_I1),R_Y      ; Restore R_Y
          move B10,l:(R_I1)P_L   ; save phase in memory argument
	  move R_I1,x:LARGS      ; save X_LARGS pointer
          endm
