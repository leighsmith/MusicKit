...
;;      l:(LARGS)P_L     current period           per0h,per0l
...
;;      See /usr/local/lib/dsp/ugsrc/test/toscg5.asm for an example of generating
...
;;      per  = l:(LARGS)P_L;         /* waveform period */
;;      phs  = l:(LARGS)P_L;         /* current count-down value */
;;
;;      for (n=0;n<I_NTICK;n++) {
;;           phs1 -= 1;
;;           if (phs1<0) {
...
;;
;;  SOURCE
;;      /usr/local/lib/dsp/ugsrc/impulses.asm
;;
;;  ALU REGISTER USE
;;      Y1 = Amplitude or 0
...
; Set up data alu regs from state held in memory arguments
          ...
          move l:(R_I1)P_L,X        ; X = current period
          clr A l:(R_I1),B          ; B = current phase
...
_done
          ...
          move B,l:(R_I1)P_L             ; save phase in memory argument 
	  move R_I1,x:LARGS
          endm
