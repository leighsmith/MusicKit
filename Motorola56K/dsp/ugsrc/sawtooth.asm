...
;;      l:(LARGS)P_L     current increment        inc0h,inc0l
;;      l:(LARGS)P_L     current wave amplitude   phs0h,phs0l
;;
;;  DESCRIPTION
...
;;      inc  = l:(LARGS)P_L;       /* amplitude increment */
;;      phs  = l:(LARGS)P_L;       /* instantaneous amplitude */
;;
;;      for (n=0;n<I_NTICK;n++) {
...
;;
;;  SOURCE
;;      /usr/local/lib/dsp/ugsrc/sawtooth.asm
;;
;;  SEE ALSO
;;      /usr/local/lib/dsp/ugsrc/oscs.asm - simple sinusoidal oscillator
;;
;;  ALU REGISTER USE
;;      X1 = Amplitude
;;      X0 = Zero
...
; Set up data alu regs from state held in memory arguments
          ...
          move l:(R_I1),A           ; A = current phase = ramp value
...
pf\_sawtooth_\ic\_tickloop
          move A,l:(R_I1)P_L        ; A = next phase = next ramp value
          move R_I1,x:LARGS	   ; Update L arg pointer
          endm
