; /usr/local/lib/dsp/smsrc/defines.asm - Symbolic definitions for various assembly constants
;
...
;;		  !HAVE_SYSTEM
;; 01/30/93/daj - Flushed R_L. Added R_IO2
;;
...
B__SYS_CALL		equ 9	; $000200 Got syscall. Was B__TEMPS_SAVED_ERR
...
B__BLOCK_RD_FINISH	equ 15	; $008000 Set when blocking at rd_block_host
B__HALF_SRATE		equ 16	; $010000 Set for up-sampling of output
			   ; This is only used for ssi output
...
B__SSI_RD_RUNNING 	equ 14  ; $004000 1 after read_data_buf._in 
...
;; Y_DEVSTAT needs to be a short (6bit) absolute address so that the JSSET
;;	     instruction will work

;; Y_DEVSTAT   dc 0	        ; Initial DEV state (defined in sys_li.asm)
; Multiple DSP support. Inter-process sound (IPS)
B__IPS_ENABLE 	        equ 0	; $000001 reading/writing from hub/satellite DSP
B__IPS_RUNNING 	        equ 1 	; $000002 inter-dsp sound running
; DRAM support
B__DRAM_ACCESSING       equ 2   ; $000004 Set if we're in our dram access macros
B__DRAM_AUTOREFRESH     equ 3   ; $000008 Set if we want auto-refresh
...
;*** R_IO was called R_DMA before ioequ.asm started being used:
	  define R_IO  'r7'   ; sound output read-or-write pointer
	  define R_IO2 'r2'   ; sound input system pointer
	  ...
	  define N_IO2 'n2'   ; sound input system pointer
...
;*** N_IO was called N_DMA before ioequ.asm started being used:
	  define N_IO  'n7'   ; sound output read-or-write pointer
	  ...
	  define M_IO2 'm2'   ; sound input system pointer
	  define M_O   'm6'   ; output signal pointer
	  define M_HMS 'm3'   ; host message stack write pointer
	  define M_IO  'm7'   ; sound output read-or-write pointer
;*** M_DMA is defined in Motorola's ioequ.asm as the DMA bit of the HSR

; MNEMONICS FOR SPECIAL ADDRESSES USED BY UNIT GENERATORS
; (These must stay in synch with sys_li.asm!)
	  define LARGS '$000004' ; An x: memory location
	  define DEVSTAT '$000004' ; A y: memory location (used for qp dram, etc.)
...
I_ALIVE_PERIOD  EQU  $008000		; period for "alive" polling.
					; This is every .75 seconds @ 44khz,
					; every 1.5 seconds @ 22 khz, etc.
					; This must be kept in synch with 
					; DSPObject.c.	
; Maybe some day
...
define_debug_halt macro
	...
	if ((QP_HUB)||(QP_SAT))            ; QP_HUB OR QP_SAT
	include 'qp.asm'
	endif
...
SC_W_REQ_0	equ	$020000	  ;"Sys call" requesting DMA write on chan 0
SC_R_REQ_0	equ	$010000	  ;"Sys call" requesting DMA write on chan 0
