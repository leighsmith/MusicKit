...
; unless PINNACLE is set, in which case x and p are overlaid.

        if (PARTIAL_OVERLAY)

; ----- EXTERNAL USER MEMORY SIZES -----

NPXE_SYS equ NXE_SYS+NPE_SYS+N_SHADOW	; All external sys memory in P or X
NPE_USR	equ I_NPE_USR			; No. off-chip words for user code
NXE_USR	equ NE_RAM-NPE_USR-NPXE_SYS	; No. off-chip words for user x data
NLE_USR	equ 0				; No. off-chip words for user l data
...
PHE_USR	  equ  PLE_USR+NPE_USR-1
LLE_USR	  equ  YLE_RAM
LHE_USR	  equ  LLE_USR+NLE_USR-1	  ; not used
XLE_USR	  equ  PHE_USR+1
XHE_USR	  equ  XLE_USR+NXE_USR-1
YLE_USR	  equ  YLE_RAM
YHE_USR	  equ  YLE_USR+NYE_USR-1

; ----- EXTERNAL SYSTEM MEMORY -----
PLE_SYS	  equ  XHE_USR+1		  ; Lo external system p address
PHE_SYS	  equ  PLE_SYS+NPE_SYS-1	  ; Hi external system p address
XLE_SYS	  equ  PHE_SYS+1		  ; Lo external system x address
XHE_SYS	  equ  XLE_SYS+NXE_SYS-1	  ; Hi external system x address
...
YHE_SYS	  equ  YLE_SYS+NYE_SYS-1	  ; Hi external system y address
	...
	else ; NOT ONE_MEM AND NOT PINNACLE

; ----- EXTERNAL USER MEMORY SIZES -----

NPE_USR	equ PHE_RAM-NPE_SYS-NPI_RAM	; p memory not overlaid with anything
NLE_USR	equ 0				; No. off-chip words for user l data
