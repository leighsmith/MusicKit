...
fft(trnsfrm_dir, npnt, window,
    source_buf, source_form, source_scale,
    result_buf, result_form, result_scale)


int               trnsfrm_dir, npnt, window;
float             *source_buf, *result_buf;
int               source_form, source_scale;
int               result_form, result_scale;


/* modifies: result_buf
   effects:  Computes npnt FFT specified by form, scale, and dir parameters.  
         Source samples (single precision float) are taken from soure_buf and the 
	 transformed representation is stored in result_buf (single precision
...
{
register  FFT_NET	*thisnet, *prevnet = NULL;
	...
	*/
	...
	thisnet=firstnet;
	while (thisnet != NULL && 			/* search linkedlist */
	   (thisnet->n != npnt || thisnet->wtype != window)) {
		prevnet = thisnet;
		thisnet = thisnet->next;
	}
	if (thisnet == NULL) {		/* if didn't find right size & wtype */
		thisnet = (FFT_NET *)malloc(sizeof(FFT_NET));	/* alloc new */
		build_fft_network(thisnet, npnt, window);	/* build net */
		if (prevnet != NULL)
			prevnet->next = thisnet;	    /* & add to list */
		else firstnet = thisnet;
	}
	load_registers(thisnet, source_buf, source_form, source_scale, 
		trnsfrm_dir);
	compute_fft(thisnet);
	store_registers(thisnet, result_buf, result_form, result_scale);
}

fft_clear()		/* Deallocate all preserved FFT networks.
			   Should use when finished with all computations. */
{
register FFT_NET	*thisnet, *nextnet;
...
build_fft_network(fft_net, n, window_type)
...
{
	 register  int cntr, i, j, s; 
	 int       stages, bps;
         int       **p, **q, *pp, *qp;
	 double     two_pi_div_n = TWO_PI / n;


	 /* network physical constraints */
	 fft_net->n   = n;
	 fft_net->bps = bps = n/2;
	 fft_net->wtype = window_type;
	 ...
	 /* calculate butterfly coefficients */ {
		  ...
		  register  double *coeffpr     = fft_net->coeffr;
		  register  double *coeffpi     = fft_net->coeffi;
		  register  double *inv_coeffpr = fft_net->inv_coeffr;
		  register  double *inv_coeffpi = fft_net->inv_coeffi;
		  ...
		  for (s = 2; s <= stages; s++) {
			   ...
			   for (i = bps/num_diff_coeffs; i > 0; i--) {
				 ...
				 *coeffpi     = -sin(two_pi_div_n*power);
				 *inv_coeffpi = sin(two_pi_div_n*power);
				 power += power_inc;
				 coeffpr++; inv_coeffpr++;
				 coeffpi++; inv_coeffpi++;
			      }
	 ...
	 */ {
		  ...
		  int       index, inc;
		  register  double **indexpr = fft_net->indexpr;
		  register  double **indexpi = fft_net->indexpi;
		  register  double **indexqr = fft_net->indexqr;
		  register  double **indexqi = fft_net->indexqi;
		  register  double *regr     = fft_net->regr;
		  register  double *regi     = fft_net->regi; 
	 ...
	 /* init to normal order indicies */ {
		  register  int *load_index,*load_indexp;
		  register  int *temp_indexp, *temp_index;
...
load_registers(fft_net, buf, buf_form, buf_scale, trnsfrm_dir)

FFT_NET           *fft_net;
float             *buf;
int               buf_form, buf_scale, trnsfrm_dir;
...

{
	 ...
	 register double *window;
	 register int index, i = 0, n = fft_net->n;
	 ...
	 case LINEAR: {
	   ...
	   case REAL: {                    /* pure REAL */
	     while (i < fft_net->n) {  
	       index = load_index[i];
	       fft_net->regr[i]=(double)buf[index] * window[index];
	       fft_net->regi[i]=0.;
	       i++;			                       
	     }
	   } break;
	   ...
	   case IMAG: {                    /* pure IMAGinary */
	     while (i < fft_net->n) {  
	       index = load_index[i];
	       fft_net->regr[i]=0;
	       fft_net->regi[i]=(double)buf[index] * window[index];
	       i++;			       
	     }                
	   } break;
	     ...
	     while (i < fft_net->n) {
	       index = load_index[i];
	       fft_net->regr[i]=(double)buf[index*2]   * window[index];
	       fft_net->regi[i]=(double)buf[index*2+1] * window[index];
	       i++;
	     }
	   } break;	 
	   ...
	   case POLAR: {                   /* magnitude followed by phase */
	     while (i < fft_net->n) {
	       index = load_index[i];
	       fft_net->regr[i]=(double)(buf[index*2] * cos(buf[index*2+1])) 
		                                      * window[index];
	       fft_net->regi[i]=(double)(buf[index*2] * sin(buf[index*2+1])) 
	                                              * window[index];
	       i++;			       
	     }                
	   } break;
	   ...
	   case REAL: {                     /* log pure REAL */
	     while (i < fft_net->n) {  
	       index = load_index[i];
	       fft_net->regr[i]=(double)pow(10., (1./20.)*buf[index]) 
	   ...
	   case IMAG: {                     /* log pure IMAGinary */
	     while (i < fft_net->n) {  
	       index = load_index[i];
	       fft_net->regr[i]=0.;
	       fft_net->regi[i]=(double)pow(10., (1./20.)*buf[index])
		    * window[index];
	       i++;			       
	    }                
	   } break;
	     ...
	     while (i < fft_net->n) {
	       index = load_index[i];
	       fft_net->regr[i]=(double)pow(10., (1./20.)*buf[index*2])
		 * window[index];
	       fft_net->regi[i]=(double)pow(10., (1./20.)*buf[index*2+1]) 
		 * window[index];
	       i++;
	     }
	   } break;	 
	   ...
	   case POLAR: {                    /* log mag followed by phase */
	     while (i < fft_net->n) {
	       ...
	       fft_net->regi[i]=(double)(pow(10., (1./20.)*buf[index*2])
...

store_registers(fft_net, buf, buf_form, buf_scale)

FFT_NET           *fft_net;
register          float *buf;
int               buf_form, buf_scale;
...
{
	 register  double **xpr, **xpi, **xqr, **xqi, *cr, *ci;
	 register  int i;
	 double    tpr, tpi, tqr, tqi;
	 int       bps = fft_net->bps;
	 ...
	 else if (fft_net->direction==INVERSE) {  /* INVERSE FFT coefficients */
...

{
	 ...
	 fft_net->regr        = (double *)malloc(n * DOUBLE_SIZE);
	 fft_net->regi        = (double *)malloc(n * DOUBLE_SIZE);
	 fft_net->coeffr      = (double *)malloc(stages*bps*DOUBLE_SIZE);
	 fft_net->coeffi      = (double *)malloc(stages*bps*DOUBLE_SIZE);
	 fft_net->inv_coeffr  = (double *)malloc(stages*bps*DOUBLE_SIZE);
	 fft_net->inv_coeffi  = (double *)malloc(stages*bps*DOUBLE_SIZE);
	 fft_net->indexpr     = (double **)malloc(stages * bps * PNTR_SIZE);
	 fft_net->indexpi     = (double **)malloc(stages * bps * PNTR_SIZE);
	 fft_net->indexqr     = (double **)malloc(stages * bps * PNTR_SIZE);
	 fft_net->indexqi     = (double **)malloc(stages * bps * PNTR_SIZE);

	 /* one dimensional load window */
	 fft_net->window      = (double *)malloc(n * DOUBLE_SIZE);
	 fft_net->inv_window  = (double *)malloc(n * DOUBLE_SIZE);
}
...
create_rectangular(window, n, scale)
...
short_to_float(short_buf, float_buf, n)

register          short *short_buf;
register          float *float_buf;
register          int n;
