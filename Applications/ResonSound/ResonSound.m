...
+ patchTemplateFor:aNote {
  /* Create the PatchTemplate if it doesn't exist. This is the specification
	...
	if (theATemplate)
	  return theATemplate;
	else theATemplate = theTemplate = [PatchTemplate new];
    } else {
	if (theBTemplate)
	  return theBTemplate;
	else theBTemplate = theTemplate = [PatchTemplate new];
    }
    ...
    envInAdd = [theTemplate addUnitGenerator:[Mul1add2UGxxxx class]];
    envelope = [theTemplate addUnitGenerator:[AsympUGy class]];
    envScale = [theTemplate addUnitGenerator:[Mul2UGxxy class]];
...
-init
{
    /* We want:
       ...
       delay = output = envScale = (delayScaler + SoundReader) * envelope
       delayScaler = delay * const
    ...
    [SE[envInAdd] setInput1:SE[xPP]];
    [SE[envInAdd] setOutput:SE[xPP]];
    [SE[envScale] setInput2:SE[yPP]];
    [SE[envScale] setInput1:SE[xPP]];
  ...
  /* A local method that applies parameters in the current Note. */ 
{
    ...
    BOOL noteHasFreq = [aNote isParPresent:MK_freq] || [aNote isParPresent:MK_keyNum];
    BOOL noteHasFeedbackGain = [aNote isParPresent:feedbackGainPar];

    /* First update instance vars. */
    if (noteHasFreq) freq = [aNote freq];
    ...
    if (isNewPhrase || noteHasFeedbackGain)
      [SE[constPP] setToConstant:DSPDoubleToFix24(feedbackGain)];
    if (isNewNote || noteHasAmpEnv || noteHasAmpAtt || 
	noteHasAmpRel) 
      MKUpdateAsymp(SE[envelope],ampEnv,0,1.0,ampAtt,ampRel,MK_NODVAL,ps);
    if (isNewNote || noteHasAmp)
      [SE[soundReader] setScale:amp];
    return self;
}
