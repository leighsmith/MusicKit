 ...
 *    return DSPAwaitHostMessage(msTimeLimit? msTimeLimit : _DSP_MACH_FOREVER);
 *  else
 ...
 * messages in complex DMA mode.
 *
 * This function is now called internally by a separate thread. 
 * To control the polling interval for DSP messages, use the following
 * function.
 */

extern int DSPMKSetMessageReaderPollingInterval(int msg_read_timeout);
/*
 * (New in release 4.0)
 *
 * Sets the sleep time (in milliseconds) between successive read attempts
 * of the DSP host interface.
 */

extern int DSPMKGetMessageReaderPollingInterval(void);
/*
 * (New in release 4.0)
 *
 * Gets the sleep time (in milliseconds) used between read attempts
 * of the DSP host interface.
 */
