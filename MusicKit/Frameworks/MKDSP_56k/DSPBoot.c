...
Modification history:
	...
	05/04/90/jos - Changed DSP_HM_GO to DSPGetSystemSymbolValue("HM_GO")
	10/31/90/jos - Inhibited DSP memory diagnostics w retry on error.
	10/27/91/jos - Strobe HF0 after loading nonstandard DSP monitor.
		       dspramtest from Ken Taylor flushed out this bug.
        1/12/96/daj -  Added resetFlag to _DSPBoot as a cache optimization.
*/

#define DSP_MEMORY_DIAGNOSTICS 1
...
static int _DSPBoot(DSPLoadSpec *system,int resetFlag)
{
    ...
    if(system->module)
	 ...
	 strncmp(system->module,"APMON",5)==0 ) 
	stdsys=1;
    
    /*
     * The DSP system must be registered BEFORE calling DSPOpenNoBoot()
     * if the DSP-to-SoundOut link is to be used.  This is because the
     * DSP system supplies the size of the available sound-buffer pool.
     */
    if (resetFlag)
      _DSPResetSystem(system);
    else DSPSetSystem(system); /* Register current DSP monitor */

    if (_DSPVerbose && !DSPIsSimulatedOnly()) 
      printf("\tBooting %s%s(%s) version %d.%d.\n",(stdsys?"":"NONSTANDARD "),
	     sysname,system->module,
    ...
    /* 
       ...
       05/07/90jos - DSPReset below (snddriver_dsp_reset) does not work.
       It causes the following DSPReadMessages to hang.
	...
	while (dr = dr->next) {
	    ...
	    if (_DSPTrace & DSP_TRACE_BOOT) 
	      printf("Loading %d words off-chip program memory at 0x%X:\n",
		     ntotalwords,(unsigned int)dr->loadAddress);
	...
	while (dr) {
	    ...
	    if (_DSPTrace & DSP_TRACE_BOOT) 
	      printf("Loading %d words of on-chip l memory at 0x%X:\n",
		     nli_sys,(unsigned int)dr->loadAddress);
...
/* SYSTEM IS LOADED AND PRESUMABLY RUNNING */
	...
	if (ec=DSPAwaitMessage(DSPGetSystemSymbolValue("DM_IDLE"),
			       10*DSPDefaultTimeLimit))
	  return _DSPError(ec, "DSPBoot: System does not claim to be alive "
			   "after bootstrap.");
	...
	if (system->startAddress) {
	    ...
	    if (_DSPVerbose) 
	      printf("\tStarting loaded image at location 0x%X.\n",
		     (unsigned int)system->startAddress);
	    if (ec=DSPReadSystemSymbolAddress(&ms,&ad,"X_START"))
	      _DSPError(ec,"DSPBoot: "
			"Could not find symbol X_START for start address");
	    else {
		if (ec=DSPWriteValue(system->startAddress,ms,ad))
		  _DSPError(ec,"DSPBoot: Could not write start address");
		if (ec=DSPHostMessage(DSPGetSystemSymbolValue("HM_GO")))
		  _DSPError(ec,"DSPBoot: DSPHostMessage 'GO' msg failed");
	    }
	} 

	if (_DSPVerbose) 
	  printf("\tDSP %s up and running.\n",_DSPCVS(DSPGetCurrentDSP()));
...
{
  return _DSPBoot(system,0);
}

int DSPReboot(DSPLoadSpec *system)
{
  return _DSPBoot(system,1);
}
