...
/*	DSPReadFile.c - load struct of type DSPLoadSpec from .lnk file
	...
	05/04/90/jos - Converted symbol name (symname) to upper case.
		       Needed by new way of getting DSP memory bounds.
	05/14/90/jos - Backed out of changing symbol names to upper case.
		       Now I am converting to upper case before the string
		       compare when looking up symbols by name. Since symbols
		       are now cached hashed by name, this only happens once.
		       As a result, case is retained but ignored in searches.
	08/24/90/daj/jos - (replace-string "[2]" "[(absasm?0:2)]" nil)
	08/23/95/jos - Added error return for pmem<0 case.
*/
    ...
    int sys;			/* TRUE to load SYSTEM and GLOBAL variables */
{
    ...
    char *tok,*record,*rp,*lnkfe,*lnkfn2,*symname;
    ...
    if (_DSPTrace & DSP_TRACE_LOAD) 
      fprintf(stderr,"Loading DSP monitor %s\n",lnkfn);
#endif

    lnkfe = _DSPGetTail(lnkfn);

    if (strcmp(lnkfe,".dsp")==0 || strcmp(lnkfe,".img")==0) {
      int ec = DSPLoadSpecReadFile(dspptr,lnkfn);
      DSP_FREE(lnkfe);
      DSP_FREE(lnkfn);
      return ec;
    }
	...
	if (strcmp(lnkfe,".lnk")!=0) {
	  ...
	  ec = DSPLoadSpecReadFile(dspptr,lnkfn); /* Try it anyway */
	  DSP_FREE(lnkfn);
	  DSP_FREE(lnkfe);
	  return ec;
      }
     ...
     */
    /* 
     * The following two lines moved by DAJ from above to plug leak.
     * 1/11/96
     */
...
/* LOAD _SYMBOL RECORD */
	if (strcmp(tok,"SYMBOL")==0) { /* snarf symbol definitions */
		...
		if ( sec->symCount[symmem]
		    >= sec->symAlloc[symmem] ) {
		    sec->symAlloc[symmem] += _DSP_EXPANDSIZE;
		    DSP_REALLOC(sec->symbols[symmem],DSPSymbol,
			       sec->symAlloc[symmem]);
		}
...
/* LOAD _XREF RECORD */
	if (strcmp(tok,"XREF")==0) {
		...
		if (sec->xrefCount>=sec->xrefAlloc) {
		    sec->xrefAlloc += _DSP_EXPANDSMALL;
		    DSP_REALLOC(sec->xrefs,char*,sec->xrefAlloc);
		}
	    }
...
"XREFs are not supported.\n");
		...
		for (m=0;m<DSP_LC_NUM_P;m++) {
			...
			fprintf(stderr,"DSPReadFile: Could not resolve symbol '%s'",symname);
		      foundSymbol:
    ...
    float fval;
{
	...
	sec->symAlloc[mem] += _DSP_EXPANDSIZE;
	DSP_REALLOC(sec->symbols[mem],DSPSymbol,
		   sec->symAlloc[mem]);
    }
 ...
 */
{
 ...
 "\n*** Possible parse error in {relocatableSymbol}@0 spec ***\n\t%s\n",
    ...
    *rpp = _DSPSkipToWhite(*rpp);
    ...
    if (pmem < 0)
        return _DSPError1(DSP_EBADLNKFILE,
                   "\n*** Found relocatable data symbol '%s' in DSP .lnk file ***\n"
                   "Only LABELS in  DSP assembly programs can be relocatable.\n"
                   "Chances are you have a typo in your .asm file which is causing\n"
                   "an x,y, or l symbol to be created instead of being expanded\n"
                   "as a macro argument.  Check that your macro arguments are used\n"
                   "with correct spellings everywhere in the macro body.\n", sym);
    if (sec->fixupCount[pmem]>=sec->fixupAlloc[pmem]) {
	sec->fixupAlloc[pmem] += 20;
	DSP_REALLOC(sec->fixups[pmem],DSPFixup,
		   sec->fixupAlloc[pmem]);
    }
