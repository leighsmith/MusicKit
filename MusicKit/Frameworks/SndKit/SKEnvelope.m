...
int indexOfBreakpointAfter(double inVal, NSMutableArray *inArray)
{
    ...
    count = [inArray count];
    for (i = 0 ; i < count ; i++) {
        if ([(SKBreakpoint *)[inArray objectAtIndex:i] getXVal] > inVal)
            return i;
    }
    return -1;
}
...
- (float)lookupYForX:(double)xVal
{
    ...
    if (justAfterIndex == -1) {
        return [(SKBreakpoint*)[breakpoints objectAtIndex:count-1] getXVal];
    }
    ...
    justAfterXVal =
        [(SKBreakpoint*)[breakpoints objectAtIndex:justAfterIndex] getXVal];
    justBeforeOrEqualXVal =
        [(SKBreakpoint*)[breakpoints objectAtIndex:justAfterIndex - 1] getXVal];
    justAfterYVal =
        [(SKBreakpoint*)[breakpoints objectAtIndex:justAfterIndex] getYVal];
    justBeforeOrEqualYVal =
        [(SKBreakpoint*)[breakpoints objectAtIndex:justAfterIndex - 1] getYVal];
...
- (int) insertXValue:(double)xVal yValue:(float)yVal flags:(int)flags;
{
    ...
    count = [breakpoints count];
    for (i = 0 ; i < count ; i++) {
        tempMax = [(SKBreakpoint *)[breakpoints objectAtIndex:i] getXVal];
...
- (int) insertXValue:(double)xVal yValue:(float)yVal flags:(int)flags atBreakpoint:(int)bp
{
    int i;
    int count;
    SKBreakpoint *newBreakpoint;

    if (!breakpoints) {
        breakpoints = [[NSMutableArray alloc] init];
    }
    count = [breakpoints count];
    newBreakpoint = [[SKBreakpoint alloc] initWithX:xVal y:yVal flags:flags];
    if (bp >= count) {
