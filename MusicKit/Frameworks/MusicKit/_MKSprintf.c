...
/* Combination of doprnt.c, ecvt.c, vsprintf.c, sprintf.c. 
 *
 * Copyright 1988-1992, NeXT Inc.  All rights reserved.
 * 
 * Copied from NeXT version and modified to be thread-safe.
 *
 * However note that the API to _doprnt was changed to support only
 * vsprintf and sprintf and these have different return values from the
 * standard versions.
 *
 * History:  1990/daj     - Created
 *           11/17/92/daj - Minor change to make compiler happy
 */

#include <stdio.h>
#include <ctype.h>
#include <stdarg.h>     /* Changed from vararg. DAJ */
#include <string.h>
#include <math.h>       /* Needed for modf */
 
//extern double isnan();  /* In libsys, but in no header file. */
//extern double isinf();
...
static char *_p_dconv(); /* Defined below - DAJ */
static char *_efmt(va_list *app, char *buf, int prec, int fcode, int alt, int sign_type);
static char *_ffmt(va_list *app, char *buf, int prec, int alt, int sign_type);
static char *_gfmt(va_list *app, char *buf, int prec, int fcode, int alt, int sign_type);
static char *btod();

char*_mkecvt(double arg, int ndigits, int *decptp, int *signp, char *buff);
char*_mkfcvt(double arg, int ndigits, int *decptp, int *signp, char *buff);

static void
 _MKdoprnt(fmt, ap, strBuffer) /* Only supports sprintf or vsprintf - DAJ */
register char *fmt;
va_list ap;
char *strBuffer;
{
    ...
    begp = endp = NULL;                 /* keep the compiler happy */
    for (; ;) {
...
static char *
_efmt(va_list *app, char *buf, int prec, int fcode, int alt, int sign_type)
{
	int is_neg, decpt;
	register char *p1, *p2;
	register i;
	char ebuf[MAXDIGS+1];
	char buff1[MAXDIGS+1];
	double number;
	p1 = buff1;
	...
	p1 = _mkecvt(number, prec+1, &decpt, &is_neg, buff1);
	p2 = buf;
	if (*p1 != '0')
		decpt--;
	if (is_neg && *p1 != '0')
...
static char *
  _ffmt(va_list *app, char *buf, int prec, int alt, int sign_type)
{
	int is_neg, decpt;
	register char *p1, *p2;
	register i;
	...
	if (prec > MAXFPPREC)
		prec = MAXFPPREC;
	p1 = _mkfcvt(number, prec, &decpt, &is_neg, buff1);
	p2 = buf;
...
static char *
  _gfmt(va_list *app, char *buf, int prec, int fcode, int alt, int sign_type)
{
	int is_neg, decpt;
	register char *p1, *p2;
	register i;
...
/* Note different return values from standard lib functions  -- DAJ */

void
_MKSprintf(char *str, const char *fmt, ...)
{
	va_list ap;

	va_start(ap, fmt);
	_MKdoprnt(fmt, ap, str);
	va_end(ap);
}

/************************************** vsprintf.c ***********************/

void _MKVsprintf(char *str, const char *fmt, va_list ap)
{
    _MKdoprnt(fmt, ap, str);
}
