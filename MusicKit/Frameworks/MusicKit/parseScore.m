...
Modification history:
  ...
  12/15/89/daj - Changed binary scorefile magic to one word.
   ...
   12/04/90/daj - Fixed include file problem by making the null statement be
                 a valid statement.
   12/19/90/daj - Fixed memory leak of _binaryIndexedObjects.
   01/14/91/daj - Fixed bug in envelope lookup.
   04/26/91/daj - Added shutUp = NO in eofLongjmp cases.
   07/08/91/daj - Fixed off by one in NEXTTCHAR
   06/28/92/daj - Added "can't open file" error when samples() file can't be
                  found.
   06/31/92/daj - Changed -name into -varName to avoid conflict with Object.h
   10/30/92/daj - Changed duplicate decl from an error to a warning.
   11/17/92/daj - Minor changes to shut up compiler warnings. 
   6/26/93/daj - Changed lexan to accept symbols beginning with '_'.
...
char c;
{
    ...
    tokenSize = newSize;
    tokenEndBuf = tokenBuf + tokenSize - 1;
    return (*tokenPtr = c);
}
...
/* Grammar is as follows:
   ...
   factor:= (assign) | value | ran | envelopeValue | t
...
#define PARSINGSTACKSIZE 100

static _MKParameterUnion *
parvalStack[PARSINGSTACKSIZE];

static short
typeStack[PARSINGSTACKSIZE];
  ...
  initExpressionParser(void)
{
    ...
    if (beenHere)
      return;
    beenHere = YES;
    for (sp = &parvalStack[PARSINGSTACKSIZE-1]; sp >= parvalStack; sp--)   
...
static void loadFromStruct(register _MKScoreInStruct * scoreP)
{
    ...
    scoreStreamPointer = parseP->_pointer; //sb to help restore stream position
    scoreStreamLength = parseP->_length;//sb
    scoreStreamBuf_Base = parseP->_buf_base;//sb
...
{
    tokenVal->symbol = [scoreRPtr->_binaryIndexedObjects objectAtIndex:index-1];
//      NX_ADDRESS(scoreRPtr->_binaryIndexedObjects)[index-1]; //sb: replaced
    if (tokenVal->symbol == nullObject)
      tokenVal->symbol = nil;
    return tokenVal->symbol;
}
 ...
 * before using it as a parameter. This is already done I think.
 */
...
static id installPart(NSString *name)
{
...
static void installLocalGlobally(NSString *s,id anObj,unsigned short type)
    /* s is any name. 
...
static void setInfo(id aNote,id infoOwner)
{
    ...
    [aNote release]; /* _setInfo: does a copy. */
}
       ...
       */
{
    ...
    NSMutableString *thePath = ([name hasSuffix:@".score"]) ?
        [NSString stringWithString:name] : [name stringByAppendingPathExtension:@"score"];

    if ([(theArray=[NSData dataWithContentsOfFile:thePath]) length] > 0)
        return theArray;
    if (![thePath hasPrefix:@"/"]) {
        home = [[NSString stringWithFormat:@"~%@%@",HOMESCOREDIR,thePath]
            stringByStandardizingPath];
        if ([(theArray=[NSData dataWithContentsOfFile:home]) length] > 0)
            return theArray;
        
        local = [NSString stringWithFormat:@"%@%@",LOCALSCOREDIR,thePath];
        if ([(theArray=[NSData dataWithContentsOfFile:local]) length] > 0)
            return theArray;
        
        system = [NSString stringWithFormat:@"%@%@",SYSTEMSCOREDIR,thePath];
        if ([(theArray=[NSData dataWithContentsOfFile:system]) length] > 0)
            return theArray;
        }
