...
Modification history:
  ...
  08/30/91/daj - Flushed superfluous setting of timeTag when creating Note
		 in _MKMidiToMusicKit().
  11/17/92/daj - Minor change to shut up compiler warning.
  11/18/92/daj - Added "inclusive midi" mode that includes keyNum on noteOffs and
                 poly key pressure.
*/
...
static BOOL inclusiveMidi = NO;

void MKSetInclusiveMidiTranslation(BOOL yesOrNo)
{
    inclusiveMidi = yesOrNo;
}

BOOL MKGetInclusiveMidiTranslation(void)
{
...
#   define NOTETAG(_chan,_keyNum) (ptr->_noteTags[_chan-1] + _keyNum)
    ...
    aNote = ptr->_note = [MKGetNoteClass() new];
    ...
    switch([aNote noteType]) {
	  ...
	  int noteTag;
	  short chan = ptr->chan;
	  if (IS_OFF(ptr,chan,keyNum) && !inclusiveMidi) { /* Throw it away. */
	      ptr->_note = nil;
	      [aNote release];            
	      return nil;
	  } 
