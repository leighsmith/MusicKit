...
/* 
 ...
 MKSynthPatch->Fm1i->Fm1vi->Fm1v

  and  

 MKSynthPatch->Fm1i->Fm1
 ...
 Fm1i is a frequency modulation MKSynthPatch with arbitrary waveforms for  
...
id _MKSPGetFmNoVibTemplate(_MKSPFMNums *ugs,id oscClass) {
    id aTemplate = [MKPatchTemplate new];
    /* these UnitGenerators will be instantiated. */
    ugs->incUG   = [aTemplate addUnitGenerator:MKAsympUGyClass()];
    ugs->indUG   = [aTemplate addUnitGenerator:MKAsympUGxClass()];
    ugs->modUG   = [aTemplate addUnitGenerator:[OscgafUGxxyy class]];
    ugs->fmAddUG  = [aTemplate addUnitGenerator:[Scl1add2UGyxy class]];
    ugs->ampUG   = [aTemplate addUnitGenerator:MKAsympUGxClass()];
...
id _MKSPGetFmAllVibTemplate(_MKSPFMNums *ugs,id oscClass) {
    ...
    ugs->incUG   = [aTemplate addUnitGenerator:MKAsympUGxClass()];
