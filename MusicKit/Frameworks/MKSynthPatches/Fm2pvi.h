...
supports vibrato.   Although it does not inherit from <b>Fm1vi</b>, it
implements the same parameters, plus some of its own. 
...
In addition to the parameters described in <b>Fm11vi.rtfd</b>, the following
parameters are supported:

<b>afterTouch -</b>Note that <b>afterTouch</b>  applies here only to the cascade
modulator's index.

<b>controlChange</b> -  MIDI pan (controller 10) has the same effect as
<b>bearing</b>.  If the <b>panSensitivity</b> is 1.0, a MIDI pan value of  0 is
full left and 127 is full right.  If the value of  <b>panSensitivity</b> is less
than 1.0, the pan range is narrowed toward the center.  The value is set by the
companion parameter, <b>controlVal</b>.  

<b>controlVal</b> - See controlChange.

<b>m2Ratio</b> - Scaler on second modulator frequency.  Defaults to 2 for Fm2cvi
and Fm2cnvi.  Defaults to 1.  The resulting modulator frequency is the value of
<b>m2Ratio</b> multiplied by the freq parameter.

<b>m2Ind</b> - Index of 2nd modulator.  If there's an envelope on the index of
the 2nd modulator, this is the index when the envelope is at 1.  <b>m2Ind1</b>
is synonym for <b>m2Ind</b>.  

<b>m2IndEnv</b> - Frequency modulation index envelope for the second modulator. 


<b>m2Ind0</b> - Modulation index for the second modulator when envelope is at 0.
 m2Ind is the index when envelope is at 1.

<b>m2IndAtt</b> - Time of attack portion of second modulator index envelope in
seconds.  If this parameter is not present, the times in the envelope are used
verbatim.

<b>m2IndRel</b> - Time of release portion of second modulator index envelope in
seconds.  If this parameter is not present, the times in the envelope are used
verbatim.

<b>m2Waveform</b> - Second modulator WaveTable.  Defaults to
sine.

<b>m2Phase</b> - Initial phase in degrees of modulator wavetable.  Rarely
needed.  Defaults to 0.0.

<b>panSensitivity -</b> In the range 0.0:1.0.  Default is 1.0.

<b>velocity -</b> In addition to the usual amplitude scaling, velocity scales
the strength of  the FM modulation.  The default velocity (64) has no effect on
FM index.  
*/
...
/*!
  ...
  @discussion <i>aNote</i> is assumed to be a noteOff.  This method causes the
              MKNote's envelopes (if any) to begin its release portion and returns
