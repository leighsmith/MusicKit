...
<b>Fm2cnvi</b> is a cascade-modulator frequency modulation SynthPatch, with an interpolating-oscillator as a carrier and a noise source modulating the frequency of the two wavetable modulators.  It provides for envelopes on amplitude, frequency, and a separate envelope on each modulator's FM index, as well as an envvelope on the noise source.  It also supports vibrato.   Although it does not inherit from <b>Fm2cvi</b>, it implements the same parameters, plus some of its own. 

<img src="Images/FM2cnvi.gif"> 
