...
@implementation DBFm1vi /*sb: moved to below the 2 includes above */
...
-_setWaveform:(BOOL)newPhrase str0:(NSString *)waveStr0 str1:(NSString *)waveStr1
{
  int interp = balance;
  if sValid(waveStr0) 
      waveform0 = MKWaveTableForTimbreKey(waveStr0,freq0*cRatio,freq1*cRatio);
  if sValid(waveStr1) 
      waveform1 = MKWaveTableForTimbreKey(waveStr1,freq0*cRatio,freq1*cRatio);
