...
@implementation Fm2pnvi:MKSynthPatch
  /* FM with interpolating carrier, two parallel modulators, and noise modulation
     on the two modulators, with optional sinusoidal and/or random vibrato.
     See comments in Fm2pnvi.h */
{
...
struct ugNums {
  int svibUG, nvibUG, onepUG, addUG1, incUG, mulUG1, noiseUG, nAmpUG, mulUG2,
      ...
      ampUG, oscUG, outUG, xsig1, xsig2, ysig1, ysig2;
};
...
static id getAllVibTemplate() {
  ...
  ugs.addUG1  = [template addUnitGenerator:[Add2UGyxy class]];
  ugs.incUG   = [template addUnitGenerator:MKAsympUGxClass()];
  ugs.mulUG1  = [template addUnitGenerator:[Mul1add2UGyxyx class]];
  ugs.noiseUG = [template addUnitGenerator:[SnoiseUGy class]];
  ugs.nAmpUG  = [template addUnitGenerator:MKAsympUGxClass()];
  ugs.mulUG2  = [template addUnitGenerator:[Mul2UGxxy class]];
  ugs.addUG2  = [template addUnitGenerator:[Scl1add2UGyxy class]];
  ugs.ind1UG  = [template addUnitGenerator:MKAsympUGxClass()];
  ugs.mod1UG  = [template addUnitGenerator:[OscgafUGxxyy class]];
  ugs.ind2UG  = [template addUnitGenerator:MKAsympUGxClass()];
  ugs.mod2UG  = [template addUnitGenerator:[OscgafUGyxyy class]];
  ugs.addUG3  = [template addUnitGenerator:[Scl2add2UGxxy class]];
  ugs.addUG4  = [template addUnitGenerator:[Scl1add2UGyxy class]];
  ugs.ampUG   = [template addUnitGenerator:MKAsympUGxClass()];
  ...
  [template to:ugs.addUG1  sel:@selector(setOutput:)   arg:ugs.ysig1];
  [template to:ugs.incUG   sel:@selector(setOutput:)   arg:ugs.xsig1];
  [template to:ugs.mulUG1  sel:@selector(setInput1:)   arg:ugs.xsig1];
  [template to:ugs.mulUG1  sel:@selector(setInput2:)   arg:ugs.ysig1];
  [template to:ugs.mulUG1  sel:@selector(setInput3:)   arg:ugs.xsig1];
  [template to:ugs.mulUG1  sel:@selector(setOutput:)   arg:ugs.ysig1];
  [template to:ugs.nAmpUG  sel:@selector(setOutput:)   arg:ugs.xsig1];
  [template to:ugs.noiseUG sel:@selector(setOutput:)   arg:ugs.ysig2];
  [template to:ugs.mulUG2  sel:@selector(setInput1:)   arg:ugs.xsig1];
  [template to:ugs.mulUG2  sel:@selector(setInput2:)   arg:ugs.ysig2];
  [template to:ugs.mulUG2  sel:@selector(setOutput:)   arg:ugs.xsig1];
  [template to:ugs.addUG2  sel:@selector(setInput1:)   arg:ugs.xsig1];
  [template to:ugs.addUG2  sel:@selector(setInput2:)   arg:ugs.ysig1];
  [template to:ugs.addUG2  sel:@selector(setOutput:)   arg:ugs.ysig2];
  [template to:ugs.ind1UG  sel:@selector(setOutput:)   arg:ugs.xsig1];
  [template to:ugs.mod1UG  sel:@selector(setAmpInput:) arg:ugs.xsig1];
  [template to:ugs.mod1UG  sel:@selector(setIncInput:) arg:ugs.ysig2];
  [template to:ugs.mod1UG  sel:@selector(setOutput:)   arg:ugs.xsig2];
  [template to:ugs.ind2UG  sel:@selector(setOutput:)   arg:ugs.xsig1];
  [template to:ugs.mod2UG  sel:@selector(setAmpInput:) arg:ugs.xsig1];
  [template to:ugs.mod2UG  sel:@selector(setIncInput:) arg:ugs.ysig2];
  [template to:ugs.mod2UG  sel:@selector(setOutput:)   arg:ugs.ysig2];
  [template to:ugs.addUG3  sel:@selector(setInput1:)   arg:ugs.xsig2];
  [template to:ugs.addUG3  sel:@selector(setInput2:)   arg:ugs.ysig2];
  [template to:ugs.addUG3  sel:@selector(setOutput:)   arg:ugs.xsig1];
  [template to:ugs.addUG4  sel:@selector(setInput1:)   arg:ugs.xsig1];
...
static id getNoVibTemplate() {
  ...
  noVibUgs = &ugs;
  /* these UnitGenerators will be instantiated. */
  ugs.incUG   = [template addUnitGenerator:MKAsympUGyClass()];
  ugs.noiseUG = [template addUnitGenerator:[SnoiseUGy class]];
  ugs.nAmpUG  = [template addUnitGenerator:MKAsympUGxClass()];
  ugs.mulUG2  = [template addUnitGenerator:[Mul2UGxxy class]];
  ugs.addUG2  = [template addUnitGenerator:[Scl1add2UGyxy class]];
...
# define UG(ugname) [synthElements objectAtIndex:NUM(ugname)]

-init
{
  [self _setDefaults]; /* Don't need [super init] */
     ...
     */
{
  ...
  if ((updateIntPar(aNote,MK_afterTouch,aftertouch) |
      ...
      newPhrase)
    [UG(addUG4) setScale:(1.0-afterTouchSensitivity) +
                         afterTouchSensitivity * (double)aftertouch/127.0];
  if (((controller==MIDI_BALANCE) && updateIntPar(aNote,MK_controlVal,balance)) ||
      newPhrase) {
    [UG(addUG3) setScale1:(balance<=64) ? 0.999999 : 1.0-(double)(balance-64)/63.0];
    [UG(addUG3) setScale2:(balance>=64) ? 0.999999 : (double)balance/64.0];
  }
  return self;
}    
