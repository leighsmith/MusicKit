...
/*
 This is an atypical MKSynthPatch in that it allocates its delay memory not 
...
#define SE(_x)        [self->synthElements objectAtIndex:_x]
...
static NSMutableArray *slaveLists = nil; /* Subserviant shared objects. This is a real hack. */
		 
static id addSlaveList(id theOrch)
{
//    NSMutableArray *aList;
//    [slaveLists addObject:aList = [[NSMutableArray alloc] initWithCapacity:6]];
//    aList->numElements = 6;
//    NX_ADDRESS(aList)[0] = theOrch;
//    [aList addObject:theOrch];//sb: added this. Don't forget this retains the orch.

	_MKList *aList;
	[slaveLists addObject:aList = [[_MKList alloc] init]];
	[aList baseAddress][0] = theOrch;
    return aList;
}
    ...
    /* We have to do this because there may be multiple orchestras. */
{
    ...
    int i;
    for (i=0; i<cnt; i++) {
	innerList = [slaveLists objectAtIndex:i];
	if ([innerList objectAtIndex:0] == theOrch)
	  return innerList;
    }
...
{
//NSMutableArray *slaveList = getSlaveListFor(self->orchestra);
    _MKList *slaveList = getSlaveListFor(self->orchestra);
//    int theCount = [slaveList count];
...
//	NX_ADDRESS(slaveList)[noiseIndex] = noise;
        [slaveList baseAddress][noiseFlt1InIndex] = noiseFlt1In;
        [slaveList baseAddress][noiseFlt2InIndex] = noiseFlt2In;
        [slaveList baseAddress][noiseFlt1Index] = noiseFlt1;
        [slaveList baseAddress][noiseFlt2Index] = noiseFlt2;
        [slaveList baseAddress][noiseIndex] = noise;
	if (!(noise && noiseFlt1In && noiseFlt1 &&
	      noiseFlt2In && noiseFlt2 && self->noiseLoc)) {
	    [noise mkdealloc];
	    [noiseFlt1In mkdealloc];
	    [noiseFlt1 mkdealloc];
	    [noiseFlt2In mkdealloc];
	    [noiseFlt2 mkdealloc];
        ...
        /*sb: the following was originally before the nil check above. But you can't add nil
            objects to NSArrays, so we check first. */
//        printf("MusicKit Pluck Class: count in SlaveList = %d\n",theCount);
//        if (theCount > 1) {
//            int i;
//            for (i=0;i<(theCount-1);i++) {
//            printf("last object %d\n",theCount-i,(int)[slaveList lastObject]);
//               [slaveList removeLastObject]; /*nb this also releases them... */
//            }
//            theCount = 1;
//        }

//        if (theCount == 1) { /*sb: 1st element only, == theOrch */
//            [slaveList addObject:noiseFlt1In];
//            [slaveList addObject:noiseFlt2In];
//            [slaveList addObject:noiseFlt1];
//            [slaveList addObject:noiseFlt2];
//            [slaveList addObject:noise];
//            }
  ...
  /* Sent by this class on object creation and reset. */
{
    ...
    self->bearing = MK_DEFAULTBEARING;      /* 0.0 degrees */
...
-preemptFor: (MKNote *) aNote
{
    /* This is unnecessary but if it's changed, the logic in updateParams
       must change too. */
    [delayMem mkdealloc];          /* Now free up memory. */
    delayMem = nil;
...
-init
{
	...
	slaveLists = [[NSMutableArray alloc] init]; /* Mapping from DSP number to 'slave'
  ...
  /* Sent when patch goes from finalDecay to idle. */
{
	...
	if (i == 1) {
	    ...
	    [[slaveList objectAtIndex:noiseFlt1Index] mkdealloc];
            [[slaveList objectAtIndex:noiseFlt2Index] mkdealloc];
            [[slaveList objectAtIndex:noiseFlt1InIndex] mkdealloc];
            [[slaveList objectAtIndex:noiseFlt2InIndex] mkdealloc];
            [[slaveList objectAtIndex:noiseIndex] mkdealloc];
	}
    }
    setDefaults(self);  /* Set default instance variables */
    return self;
