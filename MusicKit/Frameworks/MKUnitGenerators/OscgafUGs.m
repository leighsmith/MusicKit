...
/* 
  ...
  10/21/92/daj - Changed failing alloc to try shared data to prevent a particular
                 kind of thrashing.
...
#define LOOKUP_TABLE_STR NSLocalizedStringFromTableInBundle(@"Lookup Table", _MK_ERRTAB, _MKErrorBundle(), "This string occurs in the context of a larger error message: 'Lookup Table... must be set before getting increment.'  This error is rarely if ever seen by the user.")
#define INCREMENT_STR NSLocalizedStringFromTableInBundle(@"increment", _MK_ERRTAB, _MKErrorBundle(), "This string occurs in the context of a larger error message: 'Lookup Table... must be set before getting increment.' This error is rarely if ever seen by the user.")
//sb: the following destroys the comment.
#define WAVETABLE_STR [_MKErrorBundle() localizedStringForKey:@"Oscgaf Wavetable" value:@"Wavetable" table:_MK_ERRTAB]
     ...
     */
{
	...
	synthData = [orchestra sharedSynthDataFor:anObj segment:tableSegment 
		   length:aLength type:MK_oscTable];
	if (!synthData) {
	    /* If we can't allocate the memory, try halving the length 
	       (up to a point) */
	    foundShared = NO;
	    while (!foundShared && 
		   !(synthData = [orchestra allocSynthData:tableSegment
