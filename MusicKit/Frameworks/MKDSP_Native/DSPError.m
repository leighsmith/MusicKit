...
static char *foodriver_error_list[] = {
	"sound success",
...
//extern char *mach_error_string(); // LMS redundant
static char *my_foodriver_error_string(int error)
{
    if (error <= 0)
      ...
      return "Probably a sound driver error";
}
...
int _DSPMachError(
    ...
    char *msg)
{   
    char *s = my_foodriver_error_string(error);	/* FIXME: delete "my_" */
    return _DSPError1(DSP_EMACH,
		      DSPCat(msg,"\n;; Mach/foodriver/sound error: %s"),s);
} 
...
#include "_dsp_message_names.h" /* DSP message and error mnemonic arrays */
