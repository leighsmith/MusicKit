...
Modification history
	...
	10/19/90/jos - added the following from dspwrap-10 (since new UGs in):
	   08/02/90/jos - now get warning when symbol not found in symbol table
	11/26/90/mtm - Fix leaf class file generation crasher bug #12741.
    ...
    int argc; char *argv[]; 
{
    ...
    /* Added by DAJ so that no DSP need be present */
    _DSPInitDefaults(); 
    DSPAddMappedDSP(NULL, "<dummy>");
	...
	if (srctype == UG)
	    ...
	    DSPSetAPSystemFiles();
#if 0
	The .mem file containing relocation pointers has been retired.
	It just didn't buy enough for the complexity it introduced.  
	...
	if(ugargstr!=NULL) { /* Process unit generator argument symbols */
		...
		if (!found && !retried) {
		    fprintf(stderr,"\n\
*** dsploadwrap: Could not find symbol '%s' in file '%s'.  \n\
Symbol type will default to DSPDatum, ignoring type specified in the source.\n\
Most likely, the name '%s' in the DSPWRAP ARGUMENT INFO comment field\n\
is not the same as the name given in the SECOND argument to the macro\n\
new_xarg or new_yarg.  Check this in the DSP source file %s.asm as it's\n\
probably your bug.\n\
",symname,lnkfn,symname,lnkfb);
		    
		    if (symname[strlen(symname)-1]=='0') {
			ts = _DSPCopyStr(symname);
			ts[strlen(ts)-1]='\0';
			fprintf(stderr,"\n\
A common source of this error is attempting to process old 1.0 DSP macro\n\
source code.  In release 1.0, there was a trailing '0' in the symbol name\n\
in the comment field, making it the same name as the THIRD argument to the\n\
new_xarg or new_yarg macro.  In release 2.0, the trailing '0' should NOT\n\
be appended to the name.  I will try changing '%s' to '%s' and repeat the\n\
symbol table search . . . ",symname,ts);
			retried = 1;
			symname = ts;
			goto retry_search;
	    }
		...
		if (!found && retried)
		  fprintf(stderr,"no luck!  Please look for a typo.\n\n");
		else if (found && retried)
		  fprintf(stderr,"ok!  Found symbol '%s'.\n\
However, please delete the trailing '0' in the DSPWRAP ARGUMENT INFO field.\n\
",ts,symname);
	    }
	}
