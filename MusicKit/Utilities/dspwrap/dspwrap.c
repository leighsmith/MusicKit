...
Modification history
	...
	08/26/90/jos - added "-stackable" option.
        05/22/92/jos - changed /usr/bin to /usr/local/bin
        05/22/92/jos - added "-dspRoot" option
        09/28/92/jos - local_dlw_dir used for dspimg as well as dsploadwrap
        09/29/92/jos - changed /usr/lib to /usr/local/lib
...
#define DSP_SYS_VER_C 2
#define DSP_SYS_REV_C 17
#define DSP_PLI_USR_C 0x9B
#endif
    ...
    while (argc && **argv=='-') {
        ...
        switch (**argv) {
          ...
          case 's':             /* -stackableAP */
            stackable = TRUE;
            break;
          case 'e':             /* -environmentVariableDSP path */
            dspRoot = _DSPCopyStr(*(++argv)); /* Override /usr/local/lib/dsp */
            break;
          default:
...
\t\t section USER\n", macrotype == AP ? "ap" : "music");
            ...
            if (macrotype==AP) { /* Create name.dsp from name_xxx.lnk */
                ...
                sysstr = DSPCat(DSPCat(DSPCat(DSPCat(
		      (do_local_dlw?local_dlw_dir:"/usr/local/bin"),
				      "/dspimg "),lnkfn)," "),dspfn);
                printf("%s\n",sysstr);
		...
		ugasp = ugargstr + 13; /* point to end of above init string */
		    ...
		    if (argtype[i]==dspace
			|| argtype[i]==literal /* This line added by DAJ */
			) { /* "x" or "y" space spec */
...
" ======================== DSP ASSEMBLY LOG ====================\n");
    ...
    if (do_local)
      ...
      asmStr = DSPCat(asmStr,"-I/usr/local/lib/dsp/smsrc/ ");
    if (do_local_macro)
