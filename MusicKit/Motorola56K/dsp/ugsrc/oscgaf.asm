...
;;
;;  SOURCE
;;      /usr/local/lib/dsp/ugsrc/oscgaf.asm
;;
;;  SEE ALSO
;;      /usr/local/lib/dsp/ugsrc/oscgf.asm - same as this one but without an amplitude envelope.
;;      /usr/local/lib/dsp/ugsrc/oscg.asm - same but without an amplitude or frequency envelope.
;;
;;  ALU REGISTER USE
...
; Set up data alu regs from state held in memory arguments
	  ...
	  move x:LARGS,R_I1	   ; Get L arg pointer
          ...
          clr B x:(R_X)+,N_O       ; Input amp signal pointer. Put here for now
	  ...
	  move N_O,R_I1		   ; Get input amp signal pointer
