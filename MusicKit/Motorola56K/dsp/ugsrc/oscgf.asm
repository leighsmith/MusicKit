...
;;
;;  SOURCE
;;      /usr/local/lib/dsp/ugsrc/oscgf.asm
;;
;;  SEE ALSO
;;      /usr/local/lib/dsp/ugsrc/oscg.asm - same as this one but without an input signal
;;
;;  ALU REGISTER USE
...
;!        move x:(R_X)+,<amp>      ; We use amplitude in its arg slot
          ...
          move l:(R_Y),B           ; Instantaneous phase
...
pf\_oscgf_\ic\_wave1 
          ...
          move B,l:(R_Y)P_L     ; save phase in memory argument
	  move R_Y,x:L_ARGS     ; save L arg pointer
	  move N_Y,R_Y          ; restore R_Y
	  nop                   ; in case next ug accesses R_Y as first instruction
          endm
