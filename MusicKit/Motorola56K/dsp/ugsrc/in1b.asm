...
;;------------------------------ DOCUMENTATION ---------------------------
;;  NAME
;;      in1b (UG macro) - Write signal vector from channel 0 of ssi read data buffer
;;
;;  SYNOPSIS
...
;;
;;  DESCRIPTION
;;      The in1b unit-generator reads a signal vector from channel 0 of the incoming
;;      stereo sound stream
;;
;;  RESTRICTIONS
;;
;;  DSPWRAP ARGUMENT INFO
;;      in1b (prefix)pf,(instance)ic,(dspace)ospc,(output)oadr,sclA
;;
...
;;
;;  SOURCE
;;      /usr/local/lib/dsp/ugsrc/in1b.asm
;;
;;
in1b macro pf,ic,ospc,oadr0,sclA0
                new_xarg pf\_in1b_\ic\_,sclA,sclA0 ; left-channel gain
                new_yarg pf\_in1b_\ic\_,oadr,oadr0 ; output address arg
...
;;
;;	****** DO NOT CHANGE THE POSITION OF THE FOLLOWING STATEMENT *****
;;	unitgenerators/In1bUG.m pokes X_IN_SKIP explicitly as the
;;	fifth word in the DSP load image.  
;;
;;***	        move x:X_I_CHAN_OFFSET,X0 ; This symbol is not defined
	        move x:>0,N_I1     	  

;;
;;	****** DO NOT CHANGE THE POSITION OF THE FOLLOWING STATEMENT *****
;;	unitgenerators/In1bUG.m pokes X_I_SFRAME_R explicitly as the
;;	seventh word in the DSP load image.  
;;
;;***	        move x:X_I_SFRAME_R,X0  ; This symbol is not defined
	        move x:>0,N_I2		; Put it here for now
		move (R_I1)+N_I1          ; dummy read of channel A
		move N_I2,N_I1
                move  y:(R_Y)+,R_O        ; output address vector to R_O
                move y:(R_I1)+N_I1,X0     ; read sample
                do #I_NTICK,pf\_in1b_\ic\_loop
