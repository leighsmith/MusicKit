...
#   define ERROR6 "_DSPMemMapRead: Memory-map file gives no offset for space %s"
	...
	if(memmap->userOffsets[i]==DSP_UNKNOWN) { 
	    /* never specified in .mem file */
...
==============================================================================
NOTE: This function is no longer used as of 3.0 because map files are no longer
      supported since we started supporting multiple DSP memory configurations.
      The map file is only needed when using .lnk files to provide the monitor,
      and this is not being done any more.  (We use .lod --> .dsp files.)

      The function is retained for backward compatibility, but instead of
      using the "system map file", it uses the 1.0 file mkmon8k.mem or
      apmon8k.mem, depending on the module name.
==============================================================================
...
int _DSPRelocateUser(DSPLoadSpec *dsp)
/* 
 * Set up dsp->userSection->loadAddress[i].
 * The dsp struct is read from a .lnk file by _DSPLnkRead().
 */
{
    ...
    if (strncmp(dsp->module,"MK",2)==0)
      mapfile = DSPCat(DSPGetSystemDirectory(),DSP_MUSIC_SYSTEM_MEM);
    else
      mapfile = DSPCat(DSPGetSystemDirectory(),DSP_AP_SYSTEM_MEM);
