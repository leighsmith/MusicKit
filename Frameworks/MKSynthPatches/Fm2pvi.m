...
@implementation Fm2pvi:MKSynthPatch
  /* FM with interpolating carrier and two parallel modulators,
     with optional sinusoidal and/or random vibrato.
     See comments in Fm2pvi.h */
{
  double amp0, amp1, ampAtt, ampRel, freq0, freq1, freqAtt, freqRel,
         ...
         velocitySensitivity, balanceSensitivity, 
  ...
  int wavelen, volume, velocity, pan, modulation, aftertouch, balance, pitchbend;
  void *_ugNums;
}
...
struct ugNums {
  int svibUG, nvibUG, onepUG, addUG1, incUG, mulUG1,
      addUG2, ind1UG, mod1UG, ind2UG, mod2UG, addUG3,
      ampUG, oscUG, outUG, xsig1, ysig1, ysig2;
};
...
static id getAllVibTemplate() {
  ...
  ugs.mod2UG  = [template addUnitGenerator:[OscgafUGxxyy class]];
  ugs.addUG2  = [template addUnitGenerator:[Scl2add2UGxxy class]];
  ugs.addUG3  = [template addUnitGenerator:[Scl1add2UGyxy class]];
  ...
  [template to:ugs.mod1UG  sel:@selector(setOutput:)   arg:ugs.ysig2];
  ...
  [template to:ugs.mod2UG  sel:@selector(setOutput:)   arg:ugs.xsig1];
  [template to:ugs.addUG2  sel:@selector(setInput1:)   arg:ugs.xsig1];
  [template to:ugs.addUG2  sel:@selector(setInput2:)   arg:ugs.ysig2];
  [template to:ugs.addUG2  sel:@selector(setOutput:)   arg:ugs.xsig1];
  [template to:ugs.addUG3  sel:@selector(setInput1:)   arg:ugs.xsig1];
  [template to:ugs.addUG3  sel:@selector(setInput2:)   arg:ugs.ysig1];
...
-init
{
  [self _setDefaults]; /* Don't need to sent init to super */
     ...
     */
{
  ...
  if (((controller==MIDI_BALANCE) && updateIntPar(aNote,MK_controlVal,balance)) ||
      newPhrase) {
    [UG(addUG2) setScale1:(balance<=64) ? 0.999999 : 1.0-(double)(balance-64)/63.0];
    [UG(addUG2) setScale2:(balance>=64) ? 0.999999 : (double)balance/64.0];
  }
  return self;
}    
