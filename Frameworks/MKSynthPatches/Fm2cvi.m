...
@implementation Fm2cvi:MKSynthPatch
  /* FM with interpolating carrier, two cascaded modulators, and
     optional sinusoidal and/or random vibrato.
...
static id getAllVibTemplate() {
  ...
  ugs.mod2UG  = [template addUnitGenerator:[OscgafUGyxxy class]];
  ...
  ugs.addUG3  = [template addUnitGenerator:[Add2UGyxy class]];
  ...
  [template to:ugs.mod2UG  sel:@selector(setIncInput:) arg:ugs.xsig2];
...
-init
{
  [self _setDefaults]; /* Don't need to send init to super */
     ...
     */
{
  ...
  if ((updateIntPar(aNote,MK_afterTouch,aftertouch) |
      ...
      newPhrase)
    [UG(addUG2) setScale:(1.0-afterTouchSensitivity) +
