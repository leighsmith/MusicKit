...
Modification history:
	...
	1/12/96/daj -  Changed functions that malloc to ALWAYS malloc (or return NULL)
*/
...
# define ABORT(s) {fprintf(stderr,s); return(FALSE);}
# define COMPLAIN(s,p1) {fprintf(stderr,(s),(p1)); return(TRUE);}
...
FILE **ipp; char *din;
{
	...
	{
	    if (din == NULL) din = DSPCat("testin.dat",0);
...
char *_DSPGetTail(char *fn) 
/* return pointer to string equal to fn after 1st "." */
{
    char *n,*o,*removeHeadMallocedString;
...
/* printf("_DSPGetTail: /%s/ = _DSPRemoveHead(/%s/)\n",_DSPRemoveHead(fn),fn); */
    ...
    removeHeadMallocedString = fn;
    while ((*fn!=_DSP_NULLC)&&(*fn!='.'))  fn++;
    if (*fn == _DSP_NULLC) {
      DSP_FREE(removeHeadMallocedString);
      return _DSPCopyStr("");
    }
...
int def; char *name;
{
    ...
    fprintf(stderr,DSPCat(name,"(=%d):"),def);
    line = _DSPGetSN(0,0);
    white = TRUE;
    val = atoi(line);
    ...
    {
	fprintf(stderr,DSPCat(name, " set to %d\n"),val);
	return(val);
    }
}
...
# define E(name) (strlen(name)!=0)		/* `Exists' */
    ...
    if(E(dhead) && !E(nhead)) ohead = DSPCat(dhead,0); else ohead = DSPCat("",nhead);
    if(E(dbody) && !E(nbody)) obody = DSPCat(dbody,0); else obody = DSPCat("",nbody);
    if(E(dtail) && !E(ntail)) otail = DSPCat(dtail,0); else otail = DSPCat("",ntail); 
/* Special hack for trailing `.' */
    for(t=otail; *t == '.'; t++);
    if(*otail != _DSP_NULLC && (t-otail) == strlen(otail) ) otail++;

/* printf("_DSPParseName: head = /%s/, body = /%s/, tail = /%s/\n",ohead,obody,otail); */
    oname = DSPCat(ohead, s1 = DSPCat(obody, s2 = DSPCat(otail, NULL)));
    strcpy(name, oname);
    if (oname)
      DSP_FREE(oname);
    if (obody) 
...
float *ic, *oc;
{
    ...
    if (dname == NULL || *dname == _DSP_NULLC) dname = (char *)DSPCat("test.flt",0);
...
char *_DSPFirstReadableFile(char *fn,...)  {
  /* Changed by DAJ to always return a malloc'ed string (or NULL) */
    va_list ap;
    char* fb;			/* file body */
